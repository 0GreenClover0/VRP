    ...
    public class DragonWaterManager : MonoBehaviour
    {
        ...
        public Material BlitMaterial
        {
            get
            {
                ...
                {
                    _blitMaterial = new Material(Shader.Find(Constants.Shader.BlitShaderName));
                }
        ...
        private void ProcessRipplers(Camera camera, float dt)
        {
            ...
            for (int i = 0; i < _rippleProjectors.Count; i++)
            {
                _rippleProjectors[i].UpdateProjector(camera);
            }

            // render projectors
            for (int i = 0; i < _rippleProjectors.Count; i++)
            {
                var cmd = CommandBufferPool.Get($"Render Projector Target - {_rippleProjectors[i].gameObject.name}");
                cmd.Clear();

                _rippleProjectors[i].RenderTarget(cmd);
            ...
            {
                projector.Simulate(rippleComputeShader, rippleKernelMain);
            }
        }
