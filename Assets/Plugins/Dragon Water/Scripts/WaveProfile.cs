...
namespace DragonWater
{
    ...
    public class WaveProfile : ScriptableObject
    {
        ...
        internal void DispatchWaveCompute(ComputeShader shader, int kernel, Camera camera)
        {
            ...
            var offset = GetCameraOffset(camera.transform);
            ...
            shader.SetFloat(Constants.Shader.Property.ComputeTextureSizeInv, 1.0f / textureSize);
            shader.SetVector(Constants.Shader.Property.ComputeCameraOffset, offset);
            ...
            shader.SetFloat(Constants.Shader.Property.ComputeProjectionSizeInv, 1.0f / GetSnappedProjectionSize());
        ...
        internal void ConfigureRippler(ComputeShader shader, int kernel, bool isFlat, bool isHighPrecision)
        {
            if (isFlat)
            {
            ...
            shader.SetFloat(Constants.Shader.Property.ComputeWaveTextureSize, textureSize);
            shader.SetFloat(Constants.Shader.Property.ComputeWaveProjectionSize, GetSnappedProjectionSize());
            shader.SetFloat(Constants.Shader.Property.ComputeWaveProjectionSizeInv, 1.0f / GetSnappedProjectionSize());
            shader.SetFloat(Constants.Shader.Property.CumputeMaxDepth, 1.0f / rippleMaxDepth);
            shader.SetFloat(Constants.Shader.Property.CumputeRippleTime, 1.0f / rippleTime);
            shader.SetFloat(Constants.Shader.Property.CumputeRestoreTime, 1.0f / rippleRestoreTime);
        ...
        internal void ConfigureMaterial(Material material, Camera camera)
        {
            ...
            var forwardDistance = camera.transform.forward * 0.5f;
            forwardDistance.y = 0;
            var distanceVisibilityMultipplier = 1.0f + forwardDistance.magnitude;
            ...
            material.SetVector(Constants.Shader.Property.WaveTextureProjection,
                new Vector4(offset.x - realProjectionSize / 2, offset.y - realProjectionSize / 2, realProjectionSize * distanceVisibilityMultipplier, 1.0f / realProjectionSize)
                );
        }
        ...
        private Vector2 GetCameraOffset(Transform transform)
        {
            var units = GetSnappedProjectionSize() / textureSize;
            var position = transform.position + (transform.forward * projectionSize * 0.5f);
            return new Vector2(
