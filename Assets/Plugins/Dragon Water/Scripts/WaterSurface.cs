...
namespace DragonWater
{
    ...
    public class WaterSurface : MonoBehaviour
    {
        ...
        internal void UpdateWater(Camera camera)
        {
            ...
            if (!_validMesh || _gridSnapUnits == -1 || _gridFirstDistance == -1)
            {
                UpdateMesh();
            }
            ...
            if (geometryType == GeometryMeshType.InfiniteOcean)
            {
                ...
                if (attachTransform == camera.transform)
                {
                    position += (attachTransform.transform.forward * _gridFirstDistance * 0.75f);
                }
        ...
        private void UpdateMesh()
        {
            ...
            if (geometryType == GeometryMeshType.InfiniteOcean)
            {
                ...
                _gridFirstDistance = preset.lengths.Count > 0 ? preset.lengths[0] : 0;
            }
            else
            {
