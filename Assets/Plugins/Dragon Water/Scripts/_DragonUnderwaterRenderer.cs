...
namespace DragonWater
{
    ...
    public class DragonUnderwaterRenderer : MonoBehaviour
    {
        ...
        internal void UpdateUnderwater(Camera camera)
        {
            ...
            _mainLight = GameObject.FindObjectsByType<Light>(FindObjectsInactive.Exclude, FindObjectsSortMode.None)
                .FirstOrDefault(v => v.type == LightType.Directional);
#else
            _mainLight = Light.GetLights(LightType.Directional, 0).FirstOrDefault();
#endif
        ...
        private void UpdateRays(Camera camera)
        {
            ...
            if (_mainLight)
            {
                lightDirection = _mainLight.transform.forward;
            }
        ...
        private void UpdateCaustics(Camera camera)
        {
            ...
            if (_mainLight)
            {
                CausticsMaterial.SetMatrix(Constants.Shader.Property.LightDirectionMatrix, _mainLight.transform.localToWorldMatrix);
            }
