...
{
    float2 uv = (positionWS.xz - _Wave_Texture_Projection.xy) * _Wave_Texture_Projection.w;
...
RippleOutput CalculateRipple(float3 positionWS)
{
    RippleOutput output = (RippleOutput)0;
        ...
        output.amount = all(saturate(uvRipple) == uvRipple) ? output.amount : 0.0;
...
FoamOutput CalculateFoam(float3 positionWS, float3 positionOWS, float hillness, float waterDeph, RippleOutput ripple)
{
    FoamOutput output = (FoamOutput)0;
            ...
            float2 hFoam3 = SampleHillnessFoam(positionWS, positionOWS, _Foam_Noise_Texture_Extra_2, _Foam_Hillness_Params_Extra_2, hillness);
        {
            ...
            if (hFoamValue > 0.005)
            {
                float hFoamValueInv = 1.0 / hFoamValue;
                v1 = v1 * hFoamValueInv;
                v2 = v2 * hFoamValueInv;
                v3 = v3 * hFoamValueInv;
...
float4 CalculateBaseColor(float waterDepth, float hillness, RefractionOutput refraction, ColorNoiseOutput noise)
{
    float4 color;
...
MaterialOutput GetMaterialOutput(float4 color, float3 normal, float3 mousePosition, float waterConeRadius, float waterConeIntensity, float3 worldPos, float3 viewDirWS, RippleOutput ripple, FoamOutput foam, RefractionOutput refraction, ColorNoiseOutput noise)
{
    ...
    {
        halo_value = halo(float3(mousePosition.x, 0.0f, mousePosition.z), worldPos, waterConeRadius) * waterConeIntensity;
    }

    if (halo_value.a > 0.0f)
    {
        color.rgb += 1.0f * abs(halo_value.rgb) * waterConeIntensity;
    }
    ...
    if (halo_value.a > 0.0f)
    {
        output.emission.rgb = float3(0,0.01,0.125) * waterConeIntensity;
        output.emission += halo_value.a * 0.05f * waterConeIntensity;
    }
    return output;
}
...
void PatchReflections()
{
            ...
            _GlossyEnvironmentCubeMap = _Reflections_GI_Cubemap;
            sampler_GlossyEnvironmentCubeMap = sampler_Reflections_GI_Cubemap;
            _GlossyEnvironmentCubeMap_HDR.x = _Reflections_GI_Intensity;
            _GlossyEnvironmentCubeMap_HDR.y = 1;
        #else
